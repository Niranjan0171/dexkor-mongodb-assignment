...
While regex appeared faster here, it required scanning every document in the collection.
As data volume increases (millions of documents), regex-based searches degrade significantly, whereas text search continues to scale efficiently.

4. Index Design (Part 3)
...
5. MongoDB Full-Text Search (Part 4)
Text Index Creation
db.tickets.createIndex({
  subject: "text",
  description: "text",
...
Full collection scan	  Uses inverted index
No relevance ranking	  Ranked by relevance
Poor scalability	      Scales efficiently
Simple string matching	  Word-based search
...
explain/        → Before & after explain() execution outputs
